...
NUDGE = (
    ...
    "3. Review the candidates per wish and select the best match or reject all. A skill "
    "is an adaptable pattern that usually works, not a step-by-step recipe that must match "
    "your task exactly — accept a useful head-start you can tailor; reject all only when no "
    "candidate is even a pattern worth adapting (rejecting one and proceeding \"as-is\" is the "
    "same overconfidence, one stage later). Present the matches with their "
    "license/provenance/trust metadata and get the user's approval BEFORE installing "
    "anything.\n"
