...
- `--with-npx`: writes/merges `./.mcp.json` (project-scoped) — also backed up. Requires Node;
  registers the local `mcp-server/` (run `npm install` in it once). Swap to `npx -y skillfed-mcp`
  after publishing.
